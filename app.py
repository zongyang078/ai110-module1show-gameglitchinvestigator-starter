...
# Bug 1 fix: reset game when difficulty changes
...
if new_game:
    st.session_state.attempts = 0
    st.session_state.secret = random.randint(low, high)  # FIX: Use low/high instead of hardcoded 1, 100
    st.session_state.score = 0  # FIX: Reset score on new game
    st.session_state.status = "playing"
    st.session_state.history = []
...
if submit:
    ...
    # Bug 3 fix: validate guess is within the valid range
    if ok and (guess_int < low or guess_int > high):
        ok = False
        err = f"Please guess a number between {low} and {high}."

    if not ok:
        # Bug 4 fix: don't append invalid guesses to history
        st.error(err)
    else:
        st.session_state.attempts += 1
        st.session_state.history.append(guess_int)

        # FIX: Removed the bug that converted secret to string on even attempts
